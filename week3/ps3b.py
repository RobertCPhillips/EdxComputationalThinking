...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        """
        p = self.maxBirthProb * (1.0 - popDensity)
        flip = random.random()
...
class Patient(object):
    ...
    def update(self):
        ...
        integer)
        """
        remaining = [x for x in self.viruses if not x.doesClear()]
#        print remaining
        total = float(len(remaining))
        
        for v in remaining:
            try:
                new = v.reproduce(total/self.maxPop)
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    """
    t = 300
    averages = [0.0 for _ in range(t)]

    for _ in range(numTrials):
        viruses = [SimpleVirus(maxBirthProb, clearProb) for _ in range(numViruses)]
        patient = Patient(viruses, maxPop)
        timesteps = [patient.update() for _ in range(t)]
        averages = [averages[i]+timesteps[i] for i in xrange(len(averages))]
    
    averages = [x/numTrials for x in averages]
    
    pylab.plot(range(t), averages)
    pylab.title('Average number of viruses at time t')
    pylab.xlabel('Time')
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        remaining = [x for x in self.viruses if not x.doesClear()]
        total = float(len(remaining))
        popDens = total/self.maxPop
        childs = []
        
        for v in remaining:
            try:
                new = v.reproduce(popDens, self.drugs)
                childs.append(new)
                total = total + 1.0
            except NoChildException:
                pass
        
        self.viruses = remaining + childs
        return int(total)
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    drug = 'guttagonol'
    t = range(150)
    averages1a = [0.0 for _ in t]
    averages1b = [0.0 for _ in t]
    averages2a = [0.0 for _ in t]
    averages2b = [0.0 for _ in t]
    ...
    for _ in range(numTrials):
        ...
        for i in t:
            averages1a[i] += patient.update()
            averages1b[i] += patient.getResistPop([drug])

        patient.addPrescription(drug)
        
        for i in t:
            averages2a[i] += patient.update()
            averages2b[i] += patient.getResistPop([drug])
            
    averages1 = [x/numTrials for x in averages1a+averages2a]
    averages2 = [x/numTrials for x in averages1b+averages2b]
    
    pylab.plot(range(len(averages1)), averages1, label="Virus Count")
    pylab.plot(range(len(averages2)), averages2, label="Resist Count")
    pylab.title('Average number of viruses at time t')
    pylab.xlabel('Time')
    pylab.ylabel('Average Number')
...
def TestRv():
    ...
    for _ in range(10):
        result.append(virus.reproduce(0,[]).getResistances())
...
def TestRv2():
    ...
    result.append(virus.getResistances()["drug2"])
    for _ in range(100):
