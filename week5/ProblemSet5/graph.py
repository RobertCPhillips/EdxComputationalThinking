...
class WeightedEdge(Edge):
    def __init__(self, src, dest, totalWeight, outWeight):
    ...
    def __str__(self):
        return '{0}->{1} ({2}, {3})'.format(self.src, self.dest, self.totalWeight, self.outWeight)
...
class WeightedDigraph(Digraph):
    ...
    def __str__(self):
        ...
        for src in self.edges:
            for target in self.edges[src]:
                dest = target[0]
                tot = target[1][0]
                out = target[1][1]
                res = '{0}{1}->{2} ({3}, {4})\n'.format(res, src, dest, tot, out)
        return res[:-1]        